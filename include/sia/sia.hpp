...
public:
  ...
  virtual std::shared_ptr<Sia>
  EmbedBytes(const std::vector<uint8_t> &bytes) = 0;
  ...
  virtual std::shared_ptr<Sia>
  AddByteArrayN(const std::vector<uint8_t> &bytes) = 0;
  virtual std::vector<uint8_t> ReadByteArrayN(uint64_t length) = 0;
  virtual std::shared_ptr<Sia>
  AddByteArray8(const std::vector<uint8_t> &bytes) = 0;
  virtual std::vector<uint8_t> ReadByteArray8() = 0;
  virtual std::shared_ptr<Sia>
  AddByteArray16(const std::vector<uint8_t> &bytes) = 0;
  virtual std::vector<uint8_t> ReadByteArray16() = 0;
  virtual std::shared_ptr<Sia>
  AddByteArray32(const std::vector<uint8_t> &bytes) = 0;
  virtual std::vector<uint8_t> ReadByteArray32() = 0;
  virtual std::shared_ptr<Sia>
  AddByteArray64(const std::vector<uint8_t> &bytes) = 0;
